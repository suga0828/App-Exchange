...
export class JumboComponent implements OnInit {
  ...
  calculateShipping() {
    ...
    if (this.exchangeRate) {
      this.toReceive.amount = (this.amount * this.exchangeRate.value * ((100 - this.toReceive.tax) / 100)).toFixed(2).toString();
    } else {
      this.toReceive.amount = (this.amount *  ((100 - this.toReceive.tax) / 100)).toFixed(2).toString();
    }
